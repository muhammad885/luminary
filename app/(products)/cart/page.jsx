...
const CartPage = () => {
  ...
  const [itemLoading, setItemLoading] = useState({}); // Track loading per item
  ...
  // Initial stock check on component mount
  useEffect(() => {
    const checkStockAvailability = async () => {
      ...
      try {
        ...
        // Create stock map for individual product stock
        const stockMap = {};
        ...
        // Identify insufficient stock items (quantity > stock)
  ...
  // Update quantity with stock check for specific item
  const updateQuantityWithStockCheck = async (id, newQuantity) => {
    setItemLoading(prev => ({ ...prev, [id]: true }));
    
    try {
      // First update the quantity in the cart
      originalUpdateQuantity(id, newQuantity);
      
      // Check stock only for this specific item
      ...
      // Update insufficient stock items
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <main className="container mt-32 mb-20 flex-1 px-4 md:px-6">
        <h1 className="text-3xl font-semibold text-[#D4AF37] mb-8">
          Your Shopping Cart
        </h1>
        ...
        {showLoading ? (
          ...
          <div className="text-center py-16 bg-gray-50 rounded-lg border border-gray-200">
            <ShoppingCart className="h-16 w-16 text-[#D4AF37]/30 mx-auto mb-4" />
            ...
            <Link href="/">
              <Button className="bg-[#D4AF37] hover:bg-[#c5a300] text-white">
                Browse Products
              </Button>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      >
                        <div className="md:col-span-6 flex items-center space-x-4">
                          ...
                          <div>
                            <Link
                              href={`/product/${item.id}`}
                              className={`font-medium ${showError ? 'text-red-600' : 'text-gray-700 hover:text-[#D4AF37]'}`}
                            >
                              {item.name}
                            </Link>
                            {!stockCheckLoading && isInsufficient && (
                          ...
                          {itemIsLoading ? (
                            ...
                            <div className="flex items-center border border-gray-200 rounded-md overflow-hidden">
                              <Button
                                variant="ghost"
                                size="icon"
                                className="rounded-none h-8 w-8 text-gray-700 hover:text-[#D4AF37] p-0"
                                onClick={() =>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm sticky top-32">
              ...
              <div className="flex justify-between py-4 font-medium text-[#D4AF37]">
                <span>Total</span>
              ...
              <Button
                ...
                className={`w-full mt-4 cursor-pointer ${stockCheckLoading || outOfStockItems.length > 0 || insufficientStockItems.length > 0 || Object.values(itemLoading).some(Boolean) ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-[#D4AF37] hover:bg-[#c5a300] text-white'}`}
              >
                {isCheckingOut ? (
                  <>
