...
async function calculateShippingFee(state, city, orderTotal) {
  try {
    console.log("[Shipping] Calculating fee for:", { state, city, orderTotal });

    const result = await getShippingConfig();
    console.log("[Shipping] Config result:", result);

    if (!result || !result.success || !result.data) {
      console.error("[Shipping] Invalid config response");
      return 1500;
    }

    const config = result.data;
    console.log("[Shipping] Using config:", JSON.stringify(config, null, 2));

    // Check if shipping is disabled
    if (config.isActive === false) {
      console.log("[Shipping] Shipping is inactive");
      return 0;
    }

    // Convert orderTotal to number
    ...
    if (
      ...
      orderTotal >= config.freeShippingThreshold
    ) {
      console.log("[Shipping] Free shipping (global threshold)");
      return 0;
    }

    // Check city-specific rules
    if (state && city && Array.isArray(config.cityPrices)) {
      const cityPrice = config.cityPrices.find(
        (cp) => cp.state === state && cp.city === city
      );

      if (cityPrice) {
        console.log("[Shipping] Found city price:", cityPrice);
        if (
          typeof cityPrice.freeShippingThreshold === "number" &&
          orderTotal >= cityPrice.freeShippingThreshold
        ) {
          console.log("[Shipping] Free shipping (city threshold)");
          return 0;
        }
        if (typeof cityPrice.price === "number") {
          console.log("[Shipping] Using city price:", cityPrice.price);
    ...
    if (state && Array.isArray(config.statePrices)) {
      const statePrice = config.statePrices.find((sp) => sp.state === state);

      if (statePrice) {
        console.log("[Shipping] Found state price:", statePrice);
        if (
          typeof statePrice.freeShippingThreshold === "number" &&
          orderTotal >= statePrice.freeShippingThreshold
        ) {
          console.log("[Shipping] Free shipping (state threshold)");
          return 0;
        }
        if (typeof statePrice.price === "number") {
          console.log("[Shipping] Using state price:", statePrice.price);
    ...
    // Use default price
    const finalPrice =
      typeof config.defaultPrice === "number" ? config.defaultPrice : 1500;
    console.log("[Shipping] Using default price:", finalPrice);
    return finalPrice;
  } catch (error) {
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex gap-2">
            <button
              className="text-gray-500 hover:text-[#D4AF37] cursor-pointer"
              onClick={(e) => {
                e.stopPropagation();
                onEdit(address);
              }}
            ...
            {!isDefault && (
              <button
                className="relative flex items-center text-xs text-[#D4AF37] hover:underline cursor-pointer group"
                onClick={(e) => {
                  e.stopPropagation();
      ...
      {expanded && (
        <div className="mt-4 text-gray-600 space-y-2">
          <p className="flex items-start">
            ...
            <span>
              <strong>Damaturu addresses:</strong> Delivered within 24 hours
              (same day if ordered before 4pm, next business day if after 4pm)
            </span>
          </p>
          <p className="flex items-start">
            <span className="text-[#D4AF37] mr-2">•</span>
            <span>
              <strong>Other locations:</strong> Delivered within 3-5 business
              days
            </span>
          </p>
          <p className="flex items-start">
...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    const calculate = async () => {
      ...
      try {
        const fee = await calculateShippingFee(
          ...
          getTotalPrice() // Use fresh cart total
        );
        setShippingFee(fee);
      } finally {
    ...
    calculate();
  }, [selectedAddress?.state, selectedAddress?.city, cartItems]); // Track specific fields
  ...
  const renderOrderSummary = () => (
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      <h2 className="text-xl font-medium text-[#D4AF37] mb-4">Order Summary</h2>
      <div className="space-y-4 divide-y divide-gray-200">
        {cartItems.map((item) => {
          const stock = productStocks[item.id] ?? Infinity;
      ...
      <div className="border-t border-gray-200 mt-4 pt-4 space-y-2">
        ...
        <div className="flex justify-between font-medium pt-2 text-[#D4AF37]">
          <span>Total</span>
          <span>
            {isCalculatingShipping || isCheckingStock ? (
  ...
  const renderPaymentButton = () => (
    <Button
      onClick={onSubmit}
      className="bg-[#D4AF37] hover:bg-[#D4AF37]/90 text-white w-full cursor-pointer"
      disabled={
        isProcessingPayment ||
        !selectedAddress ||
        isCalculatingShipping ||
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <main className="container mt-32 mb-20 flex-1 px-4 md:px-6">
        <h1 className="text-3xl font-semibold text-[#D4AF37] mb-8">Checkout</h1>
        ...
        {cartItems.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                <h3 className="text-xl font-medium text-[#D4AF37] mb-4">
                  Need Help?
                </h3>
                ...
                <div className="space-y-2 text-sm">
                  <p className="flex items-center text-gray-600">
                    <span className="text-[#D4AF37] mr-2">•</span> Phone:
                    08066014844, 08165997050
                  </p>
                </div>
              </div>
            </div>
