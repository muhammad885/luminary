...
const sanitizeForClient = (data) => {
  ...
  if (typeof data === "object" && data.constructor === Object) {
    return Object.fromEntries(
      Object.entries(data).map(([key, value]) => [key, sanitizeForClient(value)])
    );
  }
...
export const verifyPaystackPayment = async (orderId, reference) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Paystack API error: ${response.status} - ${errorJson.message}`);
      await session.abortTransaction();
      return {
        success: false,
    ...
    if (paystackOrderId && paystackOrderId !== orderId) {
      console.error(`OrderId mismatch. Expected: ${orderId}, Paystack: ${paystackOrderId}`);
      await session.abortTransaction();
      return {
        success: false,
    ...
    if (Math.abs(paidAmount - orderAmount) > 0.01) {
      console.error(`Payment amount mismatch. Expected ${orderAmount}, received ${paidAmount}`);
      await session.abortTransaction();
      return {
        success: false,
    ...
    if (!isPendingState && !isSameReference) {
      console.error(`Transaction ID mismatch. Order: ${order.transactionId}, Paystack: ${reference}`);
      await session.abortTransaction();
      return {
        success: false,
    ...
    for (const item of order.orderItems) {
      ...
      console.log(`Stock updated for ${product.name}: -${item.quantity} units`);
    }
...
export const updateOrderStatusWithTracking = async (orderId, newStatus, email='') => {
  try {
    await dbConnect();
  ...
  try {
    ...
    if (city && state && defaultShipping.cityPrices) {
      const cityMatch = defaultShipping.cityPrices.find(
        item => item.city === city && item.state === state && item.deliveryDays
      );
